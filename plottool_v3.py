# -*- coding: utf-8 -*-
"""
Created on Tue Nov 15 10:00:30 2022
...
class Plottool:
    def __init__(self,frame,xvalues,yvalues, nrow = 1, ncol = 1, xlimits = None, ylimits = None, fontsize = None):
        ...
        #     button_lock_subplot = ttk.Button(self.window2, text = 'lock current subplot', command = self.lock_plot_num)
        #     button_unlock_subplot = ttk.Button(self.window2, text = 'unlock all subplots', command = self.unlock_plot_all)
        # else:
        #     button_lock_subplot = ttk.Button(self.window2, text = 'lock current subplot', 
        #                                      command = self.lock_plot_num, state = 'disabled')
        #     button_unlock_subplot = ttk.Button(self.window2, text = 'unlock all subplots', command = self.unlock_plot_all,
        #                                        state = 'disabled')
        ...
        button_lock_subplot.grid(column = 2, row = 12)
        button_unlock_subplot.grid(column = 3, row = 12)
    ...
    def lock_plot_num(self):
        if type(self.ax) != np.ndarray:
            self.button_plot.config(state = 'disabled')
            self.action.set('Plot locked')
        else:
            if self.plot_num - 1 < 0:
                self.lock_num.append(0)
           
            else:
                self.lock_num.append(self.plot_num - 1)
                
            if len(self.lock_num) == len(self.ax):
                self.button_plot.config(state = 'disabled')
                self.action.set('All subplots are locked')
            else:
                self.action.set('Current subplot locked')
                ...
                # else:
                #     self.action.set('Current subplot locked')
    def unlock_plot_all(self):
        self.lock_num = []
        self.button_plot.config(state = 'normal')
        self.action.set('All subplots unlocked')
