...
class SO_init:
    ...
    def __init__(self, root_window, datapath, x_avg, t_avg):
        ...
        # self.ok_colors = ["#F5C710","#009E73","#E69F00","#56B4E9","#CC79A7","#F0E442"]
        self.ok_colors = ["#56B4E9", "#E69F00", "#009E73", "#F5C710", "#CC79A7",
        ...
        try:
            ...
            self.phase_05 = tk.DoubleVar(value=-58)
            self.phase_06 = tk.DoubleVar(value=-145)
            self.phase_07 = tk.DoubleVar(value=-95)
            self.phase_08 = tk.DoubleVar(value=-19)
        ...
        button_local_f = ttk.Button(self.window_so_calculate, text = 'Local freq', command = self.Localf_stft)
        ...
        button_J2_all = ttk.Button(self.window_so_discriminate, text = 'plot J2 all delay', command = self.J2_all_optimal)
        button_J_region = ttk.Button(self.window_so_discriminate, text = 'cal J region', command = self.start_J_region)
        ...
        button_J2_all.grid(column = 1, row = 4, sticky = 'w')
        button_J_region.grid(column = 2, row = 4, sticky = 'w')
        ...
        if len(self.idx_wf_0) == len(self.idx_wf_1) and len(self.idx_so_0) == len(self.idx_so_1):
            ...
            for i in range (0, len(self.idx_wf_0)):
                ...
                while idx0 >= 0 and idx1 <= len(t)-1:
                    ...
                    idx0 = idx0 - 1
                    idx1 = idx1 + 1
                self.tw_len[newkey] = np.array(self.tw_len[newkey])/2
                ...
                dp.Basic_functions.save_data(self.path+'J1 calculation results/', newkey+'_1.dat', self.tw_len[newkey], self.J[newkey])
        ...
        if len(self.idx_wf_0) == len(self.idx_wf_1) and len(self.idx_so_0) == len(self.idx_so_1):
            ...
            # calculate discriminability of basic waveforms
                for i in range (0, len(self.idx_wf_0)):
                    ...
                    self.J_base[newkey] = np.array(self.J_base[newkey])
                    self.J_log[newkey] = np.array(self.J_log[newkey])
                    dp.Basic_functions.save_data(self.path+'J3 calculation results/', newkey+'_1.dat', self.tw_len[newkey], self.J_log[newkey])
            else:
    ...
    def discrimin_nd_shift(self):
        ...
        if len(self.idx_wf_0) == len(self.idx_wf_1) and len(self.idx_so_0) == len(self.idx_so_1):
            ...
            for i in range (0, len(self.idx_wf_0)):
                ...
                self.J_base[newkey] = np.array(self.J_base[newkey])
                self.J_log[newkey] = np.array(self.J_log[newkey])
                dp.Basic_functions.save_data(self.path+'J2 calculation results/', newkey+'_1.dat', self.tw_len[newkey], self.J[newkey])
            # calculate discriminability of superoscillations
            for j in range(0, len(self.idx_so_0)):
                newkey = 'A_SO_discriminability_' + str(self.n_dis) + '_' +self.bf.find_str_common(self.filenames[self.idx_so_0[j]], self.filenames[self.idx_so_1[j]])
                self.J[newkey] = []
                self.J_base[newkey] = []
                self.J_log[newkey] = []
                self.tw_len[newkey] = []
    ...
    def fftx_stft(self, t, x, pad = 2):
        ...
        # t_auto = 0
        # zx = 0
        freq_auto, t_auto, zx = stft(x_normal, fs, nperseg = int(len(t)/3), noverlap=int(len(t)/3)-1,
                                            window = ('kaiser', 5.5), nfft = NFFT)
    ...
    def J_all_optimal(self):
        ...
        figure, ax = plt.subplots(1, 1, figsize = (90*mm, 60*mm), dpi = 300)
        ...
        time_window = long_time_window[(long_time_window>-5)&(long_time_window<5)]
        ax.set_xlim((0.1,4))
        ax.set_xlabel('$T_{Obs} (ps)$', fontsize = 10)
        ...
        for time_delays in self.so_optimal_go.all_time_delays[self.n0.get():self.n0.get()+100]:
            E1, E2 = self.so_optimal_go.get_combined_fields(time_delays, time_window)
            tw_len, J = dp.Basic_functions.array_cal_J(time_window, E1, E2)
            ax.semilogy(0.5*tw_len, J, color = '#999999', alpha = 0.3, linewidth = 1.5)
            
        n_wf = 0    
        # prop_cycle = plt.rcParams['axes.prop_cycle']
        # colors = prop_cycle.by_key()['color']
        colors = self.ok_colors
        for key1, key2, td in zip(self.interpo_wf0.keys(), self.interpo_wf1.keys(), self.so_optimal_go.all_time_delays[0]):
            E1 = self.interpo_wf0[key1](time_window-td)
            E2 = self.interpo_wf1[key2](time_window-td)
            tw_len, J = dp.Basic_functions.array_cal_J(time_window, E1, E2)
            if '6' in key1:
                ax.semilogy(0.5*tw_len, J, alpha = 1, linewidth = 1.5, color = colors[n_wf], 
                        label = '0.'+str(n_wf+5)+'3 THz')
                arb_legend.append(Line2D([], [], linewidth = 1.5, 
                                         color = colors[n_wf], label = '0.'+str(n_wf+5)+'3 THz'))
            if '5' in key1:
                ax.semilogy(0.5*tw_len, J, alpha = 1, linewidth = 1.5, color = colors[n_wf], 
                        linestyle = 'dashdot',label = '0.'+str(n_wf+5)+'2 THz')
                arb_legend.append(Line2D([], [], linewidth = 1.5, linestyle = 'dashdot',
                                         color = colors[n_wf], label = '0.'+str(n_wf+5)+'2 THz'))
            if '8' in key1:
                ax.semilogy(0.5*tw_len, J, alpha = 1, linewidth = 1.5, color = colors[n_wf], 
                        linestyle = 'dashdot',label = '0.'+str(n_wf+5)+'2 THz')
                arb_legend.append(Line2D([], [], linewidth = 1.5, linestyle = 'dashdot',
            ...
            if '7' in key1:
                ax.semilogy(0.5*tw_len, J, alpha = 1, linewidth = 1.5, color = colors[n_wf], 
                        label = '0.'+str(n_wf+5)+'2 THz')
                arb_legend.append(Line2D([], [], linewidth = 1.5, 
                                         color = colors[n_wf], label = '0.'+str(n_wf+5)+'2 THz'))
            # ax.plot(tw_len, np.log10(J), alpha = 1, linewidth = 2, label = '0.'+str(n_wf)+'2 THz')
            n_wf = n_wf+1
       
        E1, E2 = self.so_optimal_go.get_combined_fields(self.so_optimal_go.all_time_delays[0], time_window)
        tw_len, J = dp.Basic_functions.array_cal_J(time_window, E1, E2)
        ax.semilogy(0.5*tw_len, J, color = 'black', linewidth = 1.5, linestyle = 'dashed')
        arb_legend.append(Line2D([],[], linestyle = '--', 
                                 color = self.dict_ok_colors['black'],
                                 label = 'Optimized J',
                                 linewidth = 1.5
                                 )
                          )
        plt.axvline(x = 0.6, linestyle = '--', linewidth = 1.5, color = self.dict_ok_colors['purple'])
        arb_legend.append(Line2D([], [],linestyle = '--', 
                                 color = self.dict_ok_colors['purple'],
                                 label = '$T_{Obs}=T_{SO}$',
                                 linewidth = 1.5
                                 )
                          )
        ax.legend(handles = arb_legend, loc = 'upper right', fontsize = 8)
        ax.text(0.01, 0.9, '(a)', fontsize = 10, transform=ax.transAxes)
        ax.grid(True)
        canvas0.draw()   
        toolbar = NavigationToolbar2Tk(canvas0, window1)
        
    def J2_all_optimal(self):
        new_window = tk.Toplevel()
        new_window.title('All discriminability')
        window1 = ttk.Labelframe(new_window, text='plot')
        window1.grid(column=0, row=0, sticky='w')
        ...
        ax.set_xlim((-4,4))
        ax.set_xlabel('$t_{0} (ps)$', fontsize = 10)
        ax.set_ylabel('$J$', fontsize = 10)
        ax.tick_params(axis='both', labelsize = 8)
        arb_legend = []
        for time_delays in self.so_optimal_go.all_time_delays[self.n0.get():self.n0.get()+100]:
            E1, E2 = self.so_optimal_go.get_combined_fields(time_delays, time_window)
            tw_len, J = dp.Basic_functions.array_cal_J2(time_window, E1, E2)
            ax.semilogy(tw_len, J, color = '#999999', alpha = 0.3, linewidth = 1.5)
        ...
        for key1, key2, td in zip(self.interpo_wf0.keys(), self.interpo_wf1.keys(), self.so_optimal_go.all_time_delays[0]):
            ...
            tw_len, J = dp.Basic_functions.array_cal_J2(time_window, E1, E2)
            if '6' in key1:
                ax.semilogy(tw_len, J, alpha = 1, linewidth = 1.5, color = colors[n_wf], 
                        label = '0.'+str(n_wf+5)+'3 THz')
                arb_legend.append(Line2D([], [], linewidth = 1.5, 
                                         color = colors[n_wf], label = '0.'+str(n_wf+5)+'3 THz'))
        ...
        E1, E2 = self.so_optimal_go.get_combined_fields(self.so_optimal_go.all_time_delays[0], time_window)
        tw_len, J = dp.Basic_functions.array_cal_J2(time_window, E1, E2)
        ax.semilogy(tw_len, J, color = 'black', linewidth = 1.5, linestyle = 'dashed', label = 'Optimized J')
        plt.axvline(x = -0.6, linestyle = '--', color = self.dict_ok_colors['purple'])
        plt.axvline(x = 0.6, linestyle = '--', color = self.dict_ok_colors['purple'])
        arb_legend.append(Line2D([], [], linewidth = 1.5, linestyle = '--',
                                 color = self.dict_ok_colors['purple'], label = '$t_0=\pm 0.6$ ps'))
        # ax.legend(handles = arb_legend, loc = 'upper right', fontsize = 8)
        ax.text(0.01, 0.9, '(b)', fontsize = 10, transform=ax.transAxes)
        ax.grid(True)
        canvas0.draw()   
        toolbar = NavigationToolbar2Tk(canvas0, window1)    
    ...
    def start_J_region(self):
        new_window = tk.Toplevel()
        new_window.title('Calculate J for specific regions')
       
        window2 = ttk.Labelframe(new_window, text = 'editor')
       
        window2.grid(column=0, row=1, sticky='w')
        
        
        
        
        
        self.region1 = tk.DoubleVar(value = 0.1)
        self.region2 = tk.DoubleVar(value = 4.4)
        self.move = tk.DoubleVar(value = 3)
        label_region1 = ttk.Label(master = window2, text = 'Enter start time of reigon1: ')
        entry_region1 = ttk.Entry(master = window2, textvariable = self.region1)
        label_region2 = ttk.Label(master = window2, text = 'Enter start time of reigon2: ')
        entry_region2 = ttk.Entry(master = window2, textvariable = self.region2)
        label_move_step = ttk.Label(master = window2, text = 'Enter start, stop and step: ')
        entry_move = ttk.Entry(master = window2, textvariable = self.move)
        button_cal = ttk.Button(master = window2, text = 'cal J region', command = self.cal_J_region)
       
        
        label_region1.grid(column = 0, row = 0, sticky = 'w')
        entry_region1.grid(column = 1, row = 0, sticky = 'w')
        label_region2.grid(column = 0, row = 1, sticky = 'w')
        entry_region2.grid(column = 1, row = 1, sticky = 'w')
        label_move_step.grid(column = 0, row = 2, sticky = 'w')
        entry_move.grid(column = 1, row = 2, sticky = 'w')
        button_cal.grid(column = 0, row = 3, sticky = 'w')
        
        fname_sel = []
        self.fname_region1 = []
        self.fname_region2 = []
        self.fname_region_so = []
        self.fname_region_f8 = []
        self.fname_region_f7 = []
        self.fname_region_f6 = []
        self.fname_region_f5 = []
        for fname in self.filenames:
            if 'D' in fname:
                if 'SO' in fname:
                    self.fname_region_so.append(fname)
                if 'F08' in fname:
                    self.fname_region_f8.append(fname)
                if 'F07' in fname:
                    self.fname_region_f7.append(fname)
                if 'F06' in fname:
                    self.fname_region_f6.append(fname)
                if 'F05' in fname:
                    self.fname_region_f5.append(fname)
            else:
                pass
            
        self.fname_region_f8.sort(key = fdp.str_get_D)
        self.fname_region_f7.sort(key = fdp.str_get_D)
        self.fname_region_f6.sort(key = fdp.str_get_D)
        self.fname_region_f5.sort(key = fdp.str_get_D)
        self.fname_region_so.sort(key = fdp.str_get_D)
        
        
        
        # for fname in fname_sel:
        #     if 'p2' in fname:
        #         self.fname_region.append(fname)
        #     else:
        #         D_num = fdp.str_get_D(fname)
        #         if D_num < 5:
        #             self.fname_region.append(fname)
        #         else:
        #             pass
        ...
        # for fname in fname_sel:
        #     if 'p1' in fname:
        #         self.fname_region1.append(fname)
                
        #     else:
        #         self.fname_region2.append(fname)
        #         fname_cp = fname.replace('p2', 'p1')
        #         if fname_cp in fname_sel:
        #             pass
        #         else:
        #             D_num = fdp.str_get_D(fname)
        #             fname_cp = fname.replace('D'+str(D_num), 'D'+str(D_num-1))
        #             self.fname_region1.append(fname_cp)
                       
                
                    
    def cal_J_region(self):
        self.J_region1 = []
        self.J_region2 = []
        self.J_f8 = []
        self.J_f7 = []
        ...
        self.tw_fh = []
        n = 0
        newkey1 = 'Constructive Interference'
        newkey2 = 'SO'
        newkey3 = 'F08'
        newkey4 = 'F07'
        newkey5 = 'F06'
        ...
        for i in range(0,len(self.fname_region_so)-1):
            fname1 = self.fname_region_so[0]
            fname2 = self.fname_region_so[i]
            t = self.t[fname1]
            E1 = self.x[fname1]
            E2 = self.x[fname2]
            self.idx_peaks = find_peaks(abs(E1), height = 0.5*max(E1))[0]
            self.idx_peaks_reduce = self.idx_peaks - round(len(t)/2)
            idx_con0 = np.where(t>=self.region1.get())[0][0]
            idx_so0 = np.where(t>=self.region2.get())[0][0]
            idx_len = int(1.2/0.05)
            E1c_w = E1[idx_con0:idx_con0+idx_len]
            E2c_w = E2[idx_con0:idx_con0+idx_len]
            J_temp = 2*simps(abs(E1c_w-E2c_w)**2)/simps(abs(E1c_w)**2+abs(E2c_w)**2)
            self.J_region1.append((J_temp))
            E1s_w = E1[idx_so0:idx_so0+idx_len]
            E2s_w = E2[idx_so0:idx_so0+idx_len]
            J_temp = 2*simps(abs(E1s_w-E2s_w)**2)/simps(abs(E1s_w)**2+abs(E2s_w)**2)
            self.J_region2.append((J_temp))
            self.tw.append(n/10)
            n = n+2
        
        for i in range(0,len(self.fname_region_f8)-1):
            fname1 = self.fname_region_f8[0]
            fname2 = self.fname_region_f8[i]
            t = self.t[fname1]
            E1 = self.x[fname1]
            E2 = self.x[fname2]
            self.idx_peaks = find_peaks(abs(E1), height = 0.5*max(E1))[0]
            self.idx_peaks_reduce = self.idx_peaks - round(len(t)/2)
            idx0 = int(len(E1)/2)-int(0.6/0.05)
            idx_len = int(1.2/0.05)
            E1c_w = E1[idx_con0:idx_con0+idx_len]
            E2c_w = E2[idx_con0:idx_con0+idx_len]
            J_temp = 2*simps(abs(E1c_w-E2c_w)**2)/simps(abs(E1c_w)**2+abs(E2c_w)**2)
            self.J_f8.append((J_temp))
           
     
            
        for i in range(0,len(self.fname_region_f7)-1):
            fname1 = self.fname_region_f7[0]
            fname2 = self.fname_region_f7[i]
            t = self.t[fname1]
            E1 = self.x[fname1]
            E2 = self.x[fname2]
            self.idx_peaks = find_peaks(abs(E1), height = 0.5*max(E1))[0]
            ...
            E2c_w = E2[idx_con0:idx_con0+idx_len]
            J_temp = 2*simps(abs(E1c_w-E2c_w)**2)/simps(abs(E1c_w)**2+abs(E2c_w)**2)
            self.J_f7.append((J_temp))
   

            
        for i in range(0,len(self.fname_region_f6)-1):
            fname1 = self.fname_region_f6[0]
            fname2 = self.fname_region_f6[i]
            t = self.t[fname1]
            E1 = self.x[fname1]
            E2 = self.x[fname2]
            self.idx_peaks = find_peaks(abs(E1), height = 0.5*max(E1))[0]
            ...
            E2c_w = E2[idx_con0:idx_con0+idx_len]
            J_temp = 2*simps(abs(E1c_w-E2c_w)**2)/simps(abs(E1c_w)**2+abs(E2c_w)**2)
            self.J_f6.append((J_temp))
       

            
        for i in range(0,len(self.fname_region_f5)-1):
            fname1 = self.fname_region_f5[0]
            fname2 = self.fname_region_f5[i]
            t = self.t[fname1]
            E1 = self.x[fname1]
            E2 = self.x[fname2]
            self.idx_peaks = find_peaks(abs(E1), height = 0.5*max(E1))[0]
            ...
            E2c_w = E2[idx_con0:idx_con0+idx_len]
            J_temp = 2*simps(abs(E1c_w-E2c_w)**2)/simps(abs(E1c_w)**2+abs(E2c_w)**2)
            self.J_f5.append((J_temp))
    
                
        self.J_region1 = np.array(self.J_region1)
        self.J_region2 = np.array(self.J_region2)
        
        self.tw = np.array(self.tw)
        dp.Basic_functions.save_data(self.path+'J region calculation results/', newkey1+'_1.dat', 
                                         self.tw, self.J_region1)
        dp.Basic_functions.save_data(self.path+'J region calculation results/', newkey2+'_1.dat', 
                                         self.tw, self.J_region2)
        dp.Basic_functions.save_data(self.path+'J region calculation results/', newkey3+'_1.dat', 
                                         self.tw, self.J_f8)
        dp.Basic_functions.save_data(self.path+'J region calculation results/', newkey4+'_1.dat', 
                                         self.tw, self.J_f7)
        dp.Basic_functions.save_data(self.path+'J region calculation results/', newkey5+'_1.dat', 
                                         self.tw, self.J_f6)
        dp.Basic_functions.save_data(self.path+'J region calculation results/', newkey6+'_1.dat', 
                                         self.tw, self.J_f5)
        ...
        ax.plot(self.tw, self.J_region2, label = 'SO')
        ax.plot(self.tw, self.J_region1, label = 'Constructive Interference')
        ax.plot(self.tw, self.J_f8, label = '0.82 THz')
        ax.plot(self.tw, self.J_f7, label = '0.72 THz')
        ax.plot(self.tw, self.J_f6, label = '0.63 THz')
        ax.plot(self.tw, self.J_f5, label = '0.52 THz')
        ax.set_xlabel('starting axis position (mm)')
        ax.set_ylabel('Contrast J')
        ax.legend(loc = 'best')
        canvas0.draw()
